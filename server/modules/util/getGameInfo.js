...
module.exports = async (matchId, puuid, puuid2) => {
  try {
    ...
    let userStats;
    if (puuid2) {
      userStats = allData.filter(participant => participant.puuid === puuid || participant.puuid === puuid2);
    } else {
      userStats = allData.filter(participant => participant.puuid === puuid);
    }
